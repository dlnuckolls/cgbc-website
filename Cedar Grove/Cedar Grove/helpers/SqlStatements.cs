...
namespace Cedar_Grove {
  public static class SqlStatements {
    ...
    public const string SQL_GET_MINISTRY_EVENTS = "SELECT a.[Id],a.[Subject],a.[Description],ISNULL(o.[StartDate],a.[Start]) [Start],ISNULL(o.[EndDate],a.[End]) [End], [MinistryId] FROM Appointments a OUTER APPLY dbo.ExpandRecurrence(a.RecurrenceRule, CAST(GETDATE() AS DATETIME), CAST(GETDATE() + 7 AS DATETIME)) o WHERE a.[MinistryId] = {0} ORDER BY ISNULL(o.[StartDate],a.[Start]) ASC;";
    public const string SQL_GET_ALL_MINISTRY_EVENTS = "SELECT a.[Id],a.[Subject],a.[Description],ISNULL(o.[StartDate],a.[Start]) [Start],ISNULL(o.[EndDate],a.[End]) [End], [MinistryId] FROM Appointments a OUTER APPLY dbo.ExpandRecurrence(a.RecurrenceRule, CAST(GETDATE() AS DATETIME), CAST(GETDATE() + 7 AS DATETIME)) o ORDER BY ISNULL(o.[StartDate],a.[Start]) ASC;";
