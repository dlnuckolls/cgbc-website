namespace Cedar_Grove {
  public static class SqlStatements {
    ...
    public const string SQL_READ_CHURCH_MEMBER_ADDRESSES = "SELECT CONVERT(VARCHAR(50),cma.Id) 'Id', cma.Address1, cma.Address2, cma.City, cma.[State] 'StateId', s.[State], s.Abbreviation, cma.PostalCode, cma.PostalExt, cma.[Primary] FROM dbo.ChurchMemberAddress cma INNER JOIN dbo.ChurchMemberAddress_XRef cmax ON cmax.AddressId = cma.Id INNER JOIN dbo.States s ON cma.[State] = s.Id WHERE cmax.MemberId = '{0}' ORDER BY cma.Address1";
